...
function getBooksFromApi(input) {
    ...
    return axios.get(`https://www.googleapis.com/books/v1/volumes?printType=books&q=${input}`)
        .then(res => res.data.items)
        .then(volumes => {
            const volumesMap = volumes.map(volume => ({
                reviews: [],
                id: volume.id,
                ...
                listPrice: {
                    amount: utilService.getRandomIntInclusive(0, 800),
                    currencyCode: "USD",
                    isOnSale: volume.saleInfo.saleability === 'NOT_FOR_SALE' ? false : true
                }
            }))
            console.log('from api')
            storageService.saveToStorage(input, volumesMap)
            return volumesMap
        })
}
